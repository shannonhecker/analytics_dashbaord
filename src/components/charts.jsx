...
import { HxGauge } from './highcharts/HxGauge.jsx';
...
export function NovaSpark({ data, width = 80, height = 22, color, grad }) {
  ...
  return (
    <svg width={width} height={height} style={{ display: 'block' }}>
      ...
      <path d={path} stroke={col} strokeWidth="1.3" fill="none" strokeLinejoin="round" strokeLinecap="round" />
...
// Gauge uses the real HxGauge so dimensions never clip; others stay SVG
// since they're decorative (no live data / no theming per-series).
export function CardPreview({ kind, color = 'var(--accent)', value = 6.2, max = 10 }) {
  const W = 260, H = 64;
  const gid = useId();
  if (kind === 'area') {
  ...
  if (kind === 'bars') {
    ...
    return (
      <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: 'block' }}>
        ...
        {vals.map((v, i) => (
          <rect key={i} x={i * (bw + 3)} y={H - v} width={bw} height={v} rx="1.5" fill={`url(#${gid})`} />
        ))}
      </svg>
    );
  }
  if (kind === 'gauge') {
    // Real Highcharts gauge in thumb mode — no inline value text (the card
    // already displays the KPI value above this preview slot).
    return (
      <div style={{width:'100%'}}>
        <HxGauge value={value} max={max} color={color} thumb height={H}/>
      </div>
    );
  }
  if (kind === 'donut') {
