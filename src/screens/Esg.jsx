  ...
  return (
    <div style={{flex:1, overflow:'auto', padding:24, display:'flex', flexDirection:'column', gap:16}}>
      <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:16}}>
        <NovaPanel title="ESG Score" subtitle="Weighted"><HxGauge value={5.50} max={10} label="ESG" color="var(--accent)"/></NovaPanel>
        <NovaPanel title="Environmental" subtitle="Pillar"><HxGauge value={4.21} max={10} label="E" color="var(--c2)"/></NovaPanel>
        <NovaPanel title="Social" subtitle="Pillar"><HxGauge value={6.30} max={10} label="S" color="var(--c1)"/></NovaPanel>
        <NovaPanel title="Governance" subtitle="Pillar"><HxGauge value={9.28} max={10} label="G" color="var(--c3)"/></NovaPanel>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:16}}>
        <NovaPanel title="ESG Trend" subtitle="Quarterly">
          <HxBar groups={DATA.esgTrend.labels} series={DATA.esgTrend.bars} compareLine={DATA.esgTrend.line} height={240}/>
        </NovaPanel>
        <NovaPanel title="Climate Alignment" subtitle="Portfolio weight by bucket">
          <HxBar groups={['1.5°C','<2°C','>2°C','>4°C','N/R']} series={[{name:'Weight', data:[8,42,20,18,12], color:'var(--c2)'}]} height={240} showLegend={false}/>
        </NovaPanel>
      </div>
    </div>
  );
